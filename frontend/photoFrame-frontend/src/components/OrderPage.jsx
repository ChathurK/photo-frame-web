...
const OrderPage = ({ language, translations, onPageChange }) => {
  ...
  const nextStep = () => {
    ...
    if (currentStep === 4 && (!orderData.customerName || !orderData.customerWhatsapp || !orderData.customerAddress || !orderData.deliveryDate)) {
            alert('Please fill in all required fields including delivery date before proceeding.');
      return;
    }
  ...
  const sendWhatsAppSummary = async (orderData, orderId) => {
    try {
      ...
      let message = `🖼️ *New Photo Frame Order #${orderId}*\n\n` +
        `📋 *Order Details:*\n` +
      ...
      if (orderData.imageUrl) {
        setTimeout(() => {
          const imageMessage = `📸 *Order #${orderId} - Customer Image*\n\n` +
          ...
          <div className="form-step">
                    ...
                    <div className="relative">
                      ...
                      {orderData.categoryId == category.id && (
                        <div className="absolute inset-0 flex items-center justify-center bg-green-2 bg-opacity-20">
                          <div className="p-3 text-white rounded-full shadow-lg bg-green-2 animate-pulse">
            ...
            {orderData.categoryId && (
              <div className="mt-8 text-center">
                <p className="inline-block px-4 py-3 font-medium rounded-lg text-green-2 bg-green-50">
      ...
      case 2:
        return (
          <div className="space-y-6 form-step">
            ...
            <div className="form-group">
              <label className="block mb-3 font-medium text-green-3">
                {t.order?.fields?.backgroundColor || 'Background Color'} <span className="text-sm font-normal text-gray-500">(Optional)</span>
              </label>
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  <input
                    ...
                    onChange={(e) => handleInputChange('backgroundColor', e.target.value)}
                    placeholder={t.order?.fields?.selectBackgroundColor || "#ffffff or color name"}
                    className="w-full p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-2 focus:border-transparent"
                  />
                </div>
              </div>
            ...
            <div className="form-group">
              <label className="block mb-3 font-medium text-green-3">
                {t.order?.fields?.uploadImageOptional || 'Upload Your Image (Optional)'}
              </label>
              <div className="p-6 text-center transition-colors border-2 border-gray-300 border-dashed rounded-lg hover:border-green-2">
                {uploadedImage ? (
                  <div className="space-y-4">
                    <img 
                      src={uploadedImage} 
                      alt="Uploaded preview" 
                      className="max-w-full mx-auto rounded-lg shadow-md max-h-48"
                    />
                    <div className="flex justify-center space-x-4">
                      <button
                        type="button"
                        onClick={() => document.getElementById('imageUpload').click()}
                        className="px-4 py-2 text-white transition-colors rounded-lg bg-green-2 hover:bg-green-1"
                      >
                  ...
                  <div className="space-y-4">
                    <div className="text-gray-400">
                      <svg className="w-12 h-12 mx-auto" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                    ...
                    <div>
                      <button
                        type="button"
                        onClick={() => document.getElementById('imageUpload').click()}
                        className="px-6 py-3 text-white transition-colors rounded-lg bg-green-2 hover:bg-green-1"
                      >
        ...
        return (
          <div className="space-y-6 form-step">
            <div className="form-group">
              ...
              >
                <option value="">{t.order?.fields?.chooseFrameType || 'Choose frame type...'}</option>
              ...
              {!orderData.categoryId && <p className="mt-3 text-sm text-gray-500">{t.order?.fields?.pleaseSelectCategory || 'Please select a category first'}</p>}
            </div>
            ...
            <div className="form-group">
              ...
              {!orderData.frameTypeId && <p className="mt-3 text-sm text-gray-500">Please select a frame type first</p>}
            </div>
            
            <div className="form-group">
              <label className="block mb-3 font-medium text-green-3">
                Frame Color <span className="text-sm font-normal text-gray-500">(Optional)</span>
              </label>
              <select
          ...
          <div className="space-y-6 form-step">
            <div className="form-group">
              ...
              <input
                ...
                className="w-full p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-2 focus:border-transparent"
                placeholder={t.order?.fields?.enterFullName || "Enter your full name"}
              />
            </div>
            <div className="form-group">
              <label className="block mb-3 font-medium text-green-3">
              ...
              <input
                ...
                className="w-full p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-2 focus:border-transparent"
                placeholder={t.order?.fields?.enterWhatsappNumber || "Enter your WhatsApp number"}
              />
            </div>
            <div className="form-group">
              <label className="block mb-3 font-medium text-green-3">
              ...
              <textarea
                ...
                className="w-full p-4 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-green-2 focus:border-transparent"
                rows="3"
            ...
            <div className="form-group">
              ...
              <select
                ...
                className="w-full p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-2 focus:border-transparent"
              >
                <option value="">{t.order?.fields?.selectDeliveryLocation || 'Select delivery location'}</option>
                <option value="Sri Lanka">{t.order?.fields?.sriLanka || 'Sri Lanka'}</option>
                <option value="Abroad">{t.order?.fields?.abroad || 'Abroad'}</option>
              </select>
            </div>
            <div className="form-group">
              <label className="block mb-3 font-medium text-green-3">
                {t.order?.fields?.preferredDeliveryDate || 'Preferred Delivery Date'}
              </label>
              <input
                type="date"
              ...
              />
              <p className="mt-2 text-sm text-gray-500">
                📅 {t.order?.fields?.deliveryAvailableFrom || 'Delivery available from'} {(() => {
                  const minDate = new Date();
                  minDate.setDate(minDate.getDate() + 3);
                  return minDate.toLocaleDateString(language === 'si' ? 'si-LK' : 'en-US', { 
                    weekday: 'long', 
                    year: 'numeric', 
                    month: 'long', 
                    day: 'numeric' 
            ...
            <div className="form-group">
              <label className="block mb-3 font-medium text-green-3">
                {t.order?.fields?.specialInstructionsOptional || 'Special Instructions (Optional)'}
              </label>
              <textarea
                ...
                className="w-full p-4 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-green-2 focus:border-transparent"
                rows="3"
                placeholder={t.order?.fields?.anySpecialInstructions || "Any special instructions for your order"}
              />
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-6 form-step">
            <div className="p-6 rounded-lg bg-gray-50">
              <h3 className="mb-4 text-lg font-semibold text-green-3">{t.order?.fields?.orderSummary || 'Order Summary'}</h3>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between py-1">
                  <span>{t.order?.fields?.category || 'Category'}:</span>
                  <span className="font-medium">
                    {categories.find(c => c.id == orderData.categoryId)?.name || (t.order?.fields?.notSelected || 'Not selected')}
                  </span>
                </div>
                <div className="flex justify-between py-1">
                  <span>{t.order?.fields?.frameType2 || 'Frame Type'}:</span>
                  <span className="font-medium">
                    {frameTypes.find(f => f.id == orderData.frameTypeId)?.name || (t.order?.fields?.notSelected || 'Not selected')}
                  </span>
                </div>
                <div className="flex justify-between py-1">
                  <span>{t.order?.fields?.size2 || 'Size'}:</span>
                  <span className="font-medium">
                    {sizes.find(s => s.id == orderData.sizeId)?.display || (t.order?.fields?.notSelected || 'Not selected')}
                  </span>
                </div>
                <div className="flex justify-between py-1">
                  <span>{t.order?.fields?.color || 'Color'}:</span>
                  <span className="font-medium">
                    {frameColors.find(c => c.id == orderData.frameColorId)?.name || (t.order?.fields?.notSelected || 'Not selected')}
                  </span>
                </div>
                <div className="flex justify-between py-1">
                  <span>{t.order?.fields?.numberOfPersons2 || 'Number of Persons'}:</span>
                  <span className="font-medium">{orderData.numberOfPersons}</span>
                </div>
                <div className="flex justify-between py-1">
                  <span>{t.order?.fields?.backgroundColor2 || 'Background Color'}:</span>
                ...
                <div className="flex justify-between py-1">
                  <span>{t.order?.fields?.customerImage || 'Customer Image'}:</span>
                  <span className="font-medium">
                    {orderData.imageUrl ? (
                      <span className="text-green-600">
                        ✅ {t.order?.fields?.attached || 'Attached'} ({t.order?.fields?.willBeSentViaWhatsApp || 'will be sent via WhatsApp'})
                      </span>
            ...
            <div className="p-6 rounded-lg bg-blue-50">
              <h4 className="mb-4 font-semibold text-green-3">{t.order?.fields?.customerInformation || 'Customer Information'}</h4>
              <div className="space-y-2 text-sm">
                <p><strong>{t.order?.fields?.name2 || 'Name'}:</strong> {orderData.customerName}</p>
                <p><strong>{t.order?.fields?.whatsapp || 'WhatsApp'}:</strong> {orderData.customerWhatsapp}</p>
                <p><strong>{t.order?.fields?.address || 'Address'}:</strong> {orderData.customerAddress}</p>
                <p><strong>{t.order?.fields?.deliveryLocation2 || 'Delivery Location'}:</strong> {orderData.deliveryTo || (t.order?.fields?.notSpecified || 'Not specified')}</p>
                <p><strong>{t.order?.fields?.deliveryDate2 || 'Delivery Date'}:</strong> {orderData.deliveryDate ? new Date(orderData.deliveryDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : (t.order?.fields?.notSpecified || 'Not specified')}</p>
              </div>
            </div>
            
            <div className="form-group">
              <label className="block mb-3 font-medium text-green-3">
                {t.order?.fields?.specialInstructions || 'Special Instructions'} <span className="text-sm font-normal text-gray-500">({t.order?.fields?.optional || 'Optional'})</span>
              </label>
              <textarea
                value={orderData.notes}
                onChange={(e) => handleInputChange('notes', e.target.value)}
                className="w-full p-4 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-green-2 focus:border-transparent"
                rows="3"
                placeholder={t.order?.fields?.specialInstructionsPlaceholder || "Any special instructions for your order..."}
              />
            </div>

            {orderData.imageUrl && (
              <div className="p-4 border-l-4 border-green-400 rounded bg-green-50">
                <div className="flex">
                  ...
                  <div className="text-sm text-green-700">
                    ...
                    <p className="mt-1">Your uploaded image will be sent to the seller via WhatsApp along with your order details. You may also be asked to send the image directly to the seller's WhatsApp for best quality.</p>
                  </div>
                </div>
              </div>
            )}
